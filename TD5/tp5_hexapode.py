...
def envoyer_pos(x,y,z,robot,motorName):
        angles= leg_ik(x,y,z)
        print "angles:", angles
        robot.goto_position(
...
if __name__ == '__main__':
	...
	with closing(pypot.robot.from_json('robotConfig.json')) as robot:
		...
		for i in range(1,7):
			time.sleep(0.5)
			envoyer_pos(118.79, 0, -115.14,robot,"motor_"+str(i))
		...
		envoyer_angles(-50, 0, 0,robot,"motor_3")
		time.sleep(2)
		envoyer_angles(-50, 0, 0,robot,"motor_1")

		# we power off the motors
		time.sleep(10)  # we wait for 1s
		for m in robot.motors:
