...
def envoyer_pos(x,y,z):
        angles= leg_ik(x,y,z)
        print "angles:", angles
        dxl_io.set_goal_position({61:angles[0]})
        dxl_io.set_goal_position({62:angles[1]})
        dxl_io.set_goal_position({63:angles[2]})
...
if __name__ == '__main__':
    ...
    with pypot.dynamixel.DxlIO('/dev/ttyUSB0', baudrate=1000000) as dxl_io:
        ...
        #Affiche la postion du bout de la patte dans l'espace
        '''
        while True:
            time.sleep(0.5)
        ...
        '''while(1):
            envoyer_pos(170,-45,35)
            time.sleep(1)
            envoyer_pos(170,-45,0)
            time.sleep(1)
            envoyer_pos(170,-45,35)
            time.sleep(1)
            envoyer_pos(170,0,35)
            time.sleep(1)
            envoyer_pos(170,0,0)
            time.sleep(1)
            envoyer_pos(170,0,35)
        ...
        while True:
            time.sleep(0.5)
            posActuelle=recup_pos(found_ids)
            position=leg_dk(posActuelle[0],posActuelle[1],posActuelle[2])
            print "X Y Z: ", position
